...
Band 11 外部安装 Lua 仅执行以下操作：读取固件身份；校验目标资源与凭证配对；普通
...
Band 10 Pro 的 Lua 本身带 `os.execute` 且不限制 `/dev`，因此沿用改造前在真机上
验证过的流程：`/etc/build.prop` 不可读时用 `getprop` 回退；`mkdir` 准备
`/data/canopus/inbox`；向 `/dev/canopus` 提交同样的 CPC2 INSTALL，读取 36 字节
回复（payload_size=0），失败时再发 CPC1 QUERY 读取 384 字节 CPS1 状态中的
error_code。任何常驻 Supervisor 都注册 `/dev/canopus`，不要求新入口。

两代固件的精确构建号位于不同键：Band 11 为 `ro.build.id`；Band 10 Pro 的
`ro.build.id` 不含日期后缀（如 `CONBINE_LTALM078_T3.101.036`），与 target 的
`firmware_build` 相同的是 `ro.build.customer_version`。设备差异集中在
`watchfaces/module-installer-prod/src/device-<device>.lua`，生成器按设备注入；
`test_module_installer_prod.py` 从 `fwbins` 的 ROMFS `build.prop` 核对该键。
...
`watchfaces/module-installer-prod/src/device-*.lua`。
